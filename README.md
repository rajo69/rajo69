# Rajarshi Nandi

**SAP S/4HANA Integration | Applied AI and Data Science**

Based in Sheffield, UK. I build enterprise ERP integrations on SAP S/4HANA and applied AI systems: fine-tuned LLMs, grounded RAG services and reinforcement learning agents with measurable controls.

## Core Technical Pillars

### Enterprise Systems and ERP
| Area | Technologies |
|---|---|
| Core platform | SAP S/4HANA, ABAP on HANA |
| Data modelling and services | CDS Views, OData |
| User experience | SAPUI5, SAP Fiori |
| Integration | ABAP Proxies |
| Finance | Financial reconciliation controls |

### AI and Data Science
| Area | Technologies |
|---|---|
| Deep learning | PyTorch, Hugging Face Transformers |
| LLM fine-tuning | QLoRA, rsLoRA |
| Reinforcement learning | PPO |
| Retrieval | RAG with PostgreSQL and pgvector |
| Serving and infrastructure | FastAPI, PostgreSQL, Docker |

## Featured Projects

| Project | Summary |
|---|---|
| [agentic-ai-accounting](https://github.com/rajo69/agentic-ai-accounting) | Confidence-gated RL policy evaluation for autonomous accounting workflows. |
| [chronoagent](https://github.com/rajo69/chronoagent) | Time-series security monitoring for multi-agent LLM systems. |
| [ukgc-lccp-rag-api](https://github.com/rajo69/ukgc-lccp-rag-api) | Grounded RAG API over UK Gambling Commission LCCP regulation. |
| [banking77-llama-1b-lora](https://github.com/rajo69/banking77-llama-1b-lora) | Llama 3.2 1B Instruct fine-tuned with LoRA for Banking77 intent classification. |
| [Clustering-Stability-Analysis](https://github.com/rajo69/Clustering-Stability-Analysis) | Stability analysis of clustering methods on single-cell RNA-seq data. |
| [Gate-Sparsity](https://github.com/rajo69/Gate-Sparsity) | Empirical analysis of gate sparsity and functional sensitivity in gated attention. |

## Education

- **MSc Data Science**, University of Leeds
- **BTech Electronics Engineering**

## Contact

- LinkedIn: [rajarshi-nandi](https://www.linkedin.com/in/rajarshi-nandi-a77aa5214/)
